...
from fsaa.models.cae.utils import LP_BatchNorm
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, bool_masked_pos=None, k=None, v=None):
        ...
        q = q.reshape(B, N, 1, self.num_heads, -1).permute(2, 0,
                                                           3, 1, 4).squeeze(0)    # (B, N_head, N_q, dim)

        k = F.linear(input=k, weight=self.k.weight, bias=k_bias)
        k = k.reshape(B, N_k, 1, self.num_heads, -
                      1).permute(2, 0, 3, 1, 4).squeeze(0)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, is_train=True):
        ...
        if self.pos_embed is not None:
            if self.use_abs_pos_emb:
                x = x + \
                    self.pos_embed.expand(
                        batch_size, -1, -1).type_as(x).to(x.device).clone().detach()
            else:
                x = x + \
                    self.pos_embed.expand(
                        batch_size, -1, -1).type_as(x).to(x.device).clone().detach()
        ...
        if self.lin_probe:
            if self.linear_type == 'standard':
                ...
                for blk in self.attentive_blocks:
                    query_tokens = blk(query_tokens, x, 0, 0,
                                       bool_masked_pos=None, rel_pos_bias=None)
    ...
    def forward_activations(self, x, is_train=True, layer_idxs=None):
        ...
        if self.lin_probe:
            if self.linear_type == 'standard':
                ...
                for blk in self.attentive_blocks:
                    query_tokens = blk(query_tokens, x, 0, 0,
                                       bool_masked_pos=None, rel_pos_bias=None)
                return self.fc_norm(query_tokens[:, 0, :], is_train=is_train), acts
        else:   # finetune
            if self.fc_norm is not None:    # use mean pooling
                t = x[:, 1:, :]
